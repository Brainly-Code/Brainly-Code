  ...
  useEffect(() => {
    if (selectedUser) {
      readMessages({ userId, otherUserId: selectedUser.id }).then(() => {
        refetchUnread(); // update unread counts after marking as read
      });
    }
            ...
            <div className="bg-[#6B5EDD] h-full  bg-opacity-60 flex-1 p-3 sm:p-5 overflow-y-auto space-y-4">
              {
                fetchedMessages?.length != 0 ? messages.map((msg) => (
                  <div
                    key={msg.id}
                    className={`flex items-end gap-2 sm:gap-3 ${msg.senderId === userId ? "justify-end" : "justify-start"
                      }`}
                  >
                    {msg.senderId !== userId && (
                      <img
                    ...
                    <div
                      className={`px-3 py-2 rounded-lg max-w-[75%] sm:max-w-xs text-white text-sm sm:text-base ${msg.senderId === userId
