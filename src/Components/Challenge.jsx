...
import { useCompleteChallengeMutation, useGetChallengeByIdQuery, useGetChallengeInstructionsQuery, useGetChallengeSolutionsQuery } from '../redux/api/challengeSlice';
...
const Challenge = () => {
  ...
  const { data: solutions, isLoading:isLoadingSolution, error: errorGetingChallengeSolutions } = useGetChallengeSolutionsQuery(Number(id.id));
  ...
  const handleContinueSubmit = async (e) => {
    ...
    if (challenge.useInput) {
      ...
      const correctSolutions = Array.isArray(solutions)
        ? solutions.map(s => s.solution.trim().toLowerCase())
        : [];
  
      const userAnswer = userSolution.trim().toLowerCase();
  
      if (!correctSolutions.includes(userAnswer)) {
